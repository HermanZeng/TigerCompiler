...
static struct expty transExp(S_table v, S_table t, A_exp e){
	...
	case A_whileExp:
		...
		final2 = transExp(v, t, e->u.whilee.body);
		if (final2.ty->kind == Ty_int) {
	...
	case A_assignExp:
		...
		if (final4.ty->kind != final5.ty->kind) {
	...
	case A_opExp:
		...
		if (0 <= oper && oper < 4) {/* check +,-,*,/ */
			...
			if(left.ty->kind != Ty_int && left.ty->kind != Ty_int && left.ty->kind != Ty_string){
				EM_error(e->u.op.left->pos, "int or double or record-nil required");	
			}
			if (right.ty->kind != Ty_int && right.ty->kind != Ty_int && right.ty->kind !=Ty_string) {
