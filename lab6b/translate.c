...
Tr_exp Tr_simpleVar(Tr_access ac, Tr_level l) {
	Tr_level defined = ac->level;
	Tr_level used = l;
	T_exp fp = T_Temp(F_EBP());
	while (used != defined) {
		//fp = T_Mem(fp);
		fp = T_Mem(T_Binop(T_plus, fp, T_Const(8)));
		//if((int)used->parent==0) break;//use this just to pass the test
		used = used->parent;
	}

	T_exp var = F_Exp(ac->access, fp);
	return Tr_Ex(var);
}
...
void Tr_outerFrag(Tr_exp body, Tr_level level) {
	F_frag outerfrag = F_ProcFrag(unNx(body), level->frame);
	fragList = F_FragList(outerfrag, fragList);
}
...
Tr_exp Tr_forExp(Tr_exp var, Tr_exp low, Tr_exp high, Tr_exp body, Tr_exp done){
	/*T_exp value = unEx(var);
	T_exp low_exp = unEx(low);
	T_exp high_exp = unEx(high);
	T_stm body_stm = unNx(body);
	T_exp done_exp = unEx(done);
	...
	return Tr_Nx(T_Seq(T_Jump(T_Name(testl), Temp_LabelList(testl, NULL)), 
		T_Seq(T_Label(bodyl), 
		T_Seq(body_stm,
		T_Seq(T_Move(value, T_Binop(T_minus, value, T_Const(1))),
		T_Seq(T_Cjump(T_le, value, high_exp, unEx(done)->u.NAME, body_stm),
		T_Label(unEx(done)->u.NAME)))))));*/
	Temp_label again = Temp_newlabel();
	Temp_label next = Temp_newlabel();
	Temp_temp r = Temp_newtemp();
	T_stm cjump1 = T_Cjump(T_gt, unEx(var), T_Temp(r), unEx(done)->u.NAME, again);
	T_stm cjump2 = T_Cjump(T_lt, unEx(var), T_Temp(r), next, unEx(done)->u.NAME);
	T_stm adj = T_Move(unEx(var), T_Binop(T_plus, unEx(var), T_Const(1)));
	T_exp ret;
	T_stm init1 = T_Move(unEx(var), unEx(low));
	T_stm init2 = T_Move(T_Temp(r), unEx(high));
	T_stm brh = T_Seq(cjump1, T_Seq(T_Label(again), T_Seq(T_Exp(body),
		T_Seq(cjump2, T_Seq(T_Label(next), T_Seq(adj,
		T_Jump(T_Name(again), Temp_LabelList(again, NULL))))))));
	ret = Tr_Nx(T_Seq(T_Seq(init1, init2), T_Seq(brh, T_Label(unEx(done)->u.NAME))));
	return ret;
}
...
Tr_exp Tr_ifExp(Tr_exp test, Tr_exp then, Tr_exp elsee) {
	...
	if (!elsee) {/* there is no-else sentence */
		...
		/*Temp_temp r = Temp_newtemp();
		T_stm joinJump = T_Jump(T_Name(join), Temp_LabelList(join, NULL));
		result = Tr_Ex(T_Eseq(T_Move(T_Temp(r), T_Const(1)), 
			T_Eseq(cond.stm,
			T_Eseq(T_Label(f),
			T_Eseq(T_Move(T_Temp(r), T_Const(0)),
			T_Eseq(T_Exp(unEx(elsee)),
			T_Eseq(joinJump,
			T_Eseq(T_Label(t),
			T_Eseq(T_Exp(unEx(then)),
			T_Eseq(joinJump, 
			T_Eseq(T_Label(join), T_Temp(r))))))))))));*/
		/*return T_Eseq(T_Move(T_Temp(r), T_Const(1)),
			T_Eseq(e->u.cx.stm,
